<template>
  <div class="tasks-page">
    ...
    <el-card v-if="connected"><template #header><div class="section-title"><span>任务列表</span><el-button @click="loadTasks">刷新</el-button></div></template><div class="task-list"><article v-for="task in tasks" :key="task.id" class="task-row"><div><strong>{{ task.id }}</strong><p>策略 {{ task.policy_id }} · {{ task.processed }}/{{ task.total }} · 命中 {{ task.sensitive }} · 失败 {{ task.failed }}</p></div><el-tag :type="statusType(task.status)">{{ statusText(task.status) }}</el-tag><div class="actions"><el-button v-if="['queued','running'].includes(task.status)" size="small" type="danger" plain @click="cancel(task.id)">取消</el-button><el-button v-if="task.status==='completed'" size="small" @click="download(task.id,'csv')">CSV</el-button><el-button v-if="task.status==='completed'" size="small" @click="download(task.id,'jsonl')">JSONL</el-button></div></article><el-empty v-if="!tasks.length" description="暂无批量任务" /></div></el-card>
  </div>
</template>

<script setup>
import { computed, onBeforeUnmount, onMounted, ref } from 'vue'
import { cancelBatchTask, createBatchTask, describeAPIError, downloadBatchResults, fetchBatchTasks, fetchCategories, fetchPlatformPolicies, savePlatformPolicy } from '@/services/api'
...
const loadTasks=async()=>{try{tasks.value=(await fetchBatchTasks(token.value)).items||[];error.value=''}catch(err){report(err);connected.value=false}}
...
const savePolicy=async()=>{try{await savePlatformPolicy(token.value,policyForm.value);policyForm.value={...policyForm.value,id:'',name:'',categories:[]};await loadPolicies()}catch(err){report(err)}}
